...
class Index extends Controller
{
  ...
  public function __invoke(Request $request)
  {
    ...
    // If the client requests all recipients across pages, return only the specified fields
    if ($request->boolean('all_recipients')) {
      $fields = $request->input('fields', 'id');
      $fieldsArray = array_map('trim', explode(',', $fields));
      return response()->json([
        'data' => $query->get($fieldsArray)
      ]);
    }
  ...
  private function getRecipientStats(int $userId): array
  {
    ...
    return [
      ...
      'unsubscribed' => (clone $baseQuery)->unsubscribed()->count(),
      'recent' => (clone $baseQuery)->new()->count()
    ];
  }
}
